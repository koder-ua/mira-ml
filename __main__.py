...
from .data import IOMetricsWriteVda, add_iop_size, TSDBCreds, parse_db_creds
from .db import LocalDBNP, load_vms, LocalDB
from .ml import get_metric_slice, metric_distance, sort_by_hdistance, get_cross_vm_dist
from .plot import plot_cross_vm_dist, plot_metrics
...
def parse_args(argv):
    ...
    report_parser = subparsers.add_parser('ls', help='list all vms')
    sync_parser = subparsers.add_parser('sync2sqlite', help='Sync data to sqlite')
...
def main(argv):
    ...
    if opts.subparser_name == 'ls':
        ...
        # plot_vm_lifetime(vms_l, pyplot.subplot(gs[:,:]))
